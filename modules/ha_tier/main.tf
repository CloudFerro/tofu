...
resource "openstack_compute_servergroup_v2" "ha_tier" {
  name     = "ha_tier"
  policies = ["soft-anti-affinity"]
}

resource "openstack_networking_secgroup_v2" "ha_tier" {
  name                 = "ha_tier"
  description          = "HA tier security group: ingress and egress controlled."
...
resource "openstack_networking_secgroup_rule_v2" "ingress_tcp_ipv4" {
  ...
  security_group_id = openstack_networking_secgroup_v2.ha_tier.id
  region            = var.region

  depends_on = [
    openstack_networking_secgroup_v2.ha_tier
  ]
}
...
resource "openstack_networking_port_v2" "ha_tier_port" {
  count = var.vm_count
  ...
  security_group_ids = [
    openstack_networking_secgroup_v2.ha_tier.id
  ]
...
resource "openstack_compute_instance_v2" "ha_tier" {
  count = var.vm_count
  ...
  scheduler_hints {
    group = openstack_compute_servergroup_v2.ha_tier.id
  }

  network {
    port = openstack_networking_port_v2.ha_tier_port[count.index].id
  }

  depends_on = [
    openstack_compute_servergroup_v2.ha_tier,
    openstack_networking_port_v2.ha_tier_port
  ]
}

resource "openstack_networking_floatingip_v2" "ha_tier_fip" {
...
resource "openstack_networking_floatingip_associate_v2" "ha_tier_fip_assoc" {
  count = var.attach_fip ? var.vm_count : 0

  floating_ip = openstack_networking_floatingip_v2.ha_tier_fip[count.index].address
  port_id     = openstack_networking_port_v2.ha_tier_port[count.index].id
  region      = var.region

  depends_on = [
    openstack_compute_instance_v2.ha_tier,
    openstack_networking_floatingip_v2.ha_tier_fip
  ]
}

resource "openstack_lb_loadbalancer_v2" "ha_tier" {
...
resource "openstack_lb_listener_v2" "ha_tier" {
  ...
  loadbalancer_id = openstack_lb_loadbalancer_v2.ha_tier.id
  region          = var.region

  depends_on = [
    openstack_lb_loadbalancer_v2.ha_tier
  ]
}

resource "openstack_lb_pool_v2" "ha_tier" {
  name        = format("%s-pool", var.loadbalancer_name)
  protocol    = var.loadbalancer_pool_protocol
  lb_method   = var.loadbalancer_lb_method
  listener_id = openstack_lb_listener_v2.ha_tier.id
  region      = var.region

  depends_on = [
    openstack_lb_listener_v2.ha_tier
  ]
}

resource "openstack_lb_member_v2" "ha_tier" {
  count = var.vm_count

  pool_id       = openstack_lb_pool_v2.ha_tier.id
  address       = openstack_networking_port_v2.ha_tier_port[count.index].all_fixed_ips[0]
  protocol_port = var.loadbalancer_member_port
  subnet_id     = var.lb_member_subnet_id
  region        = var.region

  depends_on = [
    openstack_lb_pool_v2.ha_tier,
    openstack_networking_port_v2.ha_tier_port
  ]
}

resource "openstack_lb_monitor_v2" "ha_tier" {
  pool_id        = openstack_lb_pool_v2.ha_tier.id
...
resource "openstack_networking_floatingip_v2" "loadbalancer_vip_fip" {
  ...
  pool    = "external"
  port_id = openstack_lb_loadbalancer_v2.ha_tier.vip_port_id
  region  = var.region
